...
contains
  ...
  subroutine assign_from_mol2(mol2l,mol2r)
    ...
    type (mol2_object), intent(out) :: mol2l
    ...
    allocate(mol2l%atoms(mol2l%num_atoms))
    allocate(mol2l%order(mol2l%num_atoms))
    allocate(mol2l%connections(mol2l%num_atoms))

    mol2l%order = mol2r%order
    do i = 1, mol2l%num_atoms
       mol2l%atoms(i) = mol2r%atoms(i)
       allocate(mol2l%connections(i)%connectivity(size(mol2r%connections(i)%connectivity)))
       mol2l%connections(i)%connectivity = mol2r%connections(i)%connectivity
    end do

    allocate(mol2l%bonds(mol2l%num_bonds))
    do i = 1, mol2l%num_bonds
       mol2l%bonds(i) = mol2r%bonds(i)
    end do
    
    allocate(mol2l%substructures(mol2l%num_subst))
